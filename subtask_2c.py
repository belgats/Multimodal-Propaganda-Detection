...
def run_baselines(data_dir, test_split, train_fpath, test_fpath):
    ...
    if check_format(majority_baseline_fpath):
        ...
        logging.info(f"Majority Baseline acc (acc): {acc}")
        logging.info(f"Majority Baseline precision (): {precision}")
        logging.info(f"Majority Baseline recall (): {recall}")
    ...
    if check_format(random_baseline_fpath):
        ...
        logging.info(f"Random Baseline acc (acc): {acc}")
        logging.info(f"Random Baseline precision (): {precision}")
        logging.info(f"Random Baseline recall (): {recall}")
    ...
    if check_format(ngram_baseline_fpath):
        ...
        logging.info(f"Ngram Baseline acc (acc): {acc}")
        logging.info(f"Ngram Baseline precision (): {precision}")
        logging.info(f"Ngram Baseline recall (): {recall}")
    ...
    if check_format(imgbert_baseline_fpath):
        ...
        logging.info(f"ImgBert Baseline acc (acc): {acc}")
        logging.info(f"ImgBert Baseline precision (): {precision}")
        logging.info(f"ImgBert Baseline recall (): {recall}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--data-dir", "-d", required=False, type=str,
                        default="/home/slasher/araieval_arabicnlp24/task2/",
                        help="The absolute path to the training data")
    parser.add_argument("--test-split", "-s", required=False, type=str,
                        default="test", help="Test split name")
    ...
    parser.add_argument("--test-file-name", "-te", required=False, type=str,
                        default="arabic_memes_propaganda_araieval_24_test_gold.json",
                        help="Test file name")
